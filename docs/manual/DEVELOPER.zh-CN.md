...
> 本文档记录 Cubectl 项目的研发思路、架构设计和技术栈，以供后续开发者或代码助手继续研发。

## 🎯 项目愿景

Cubectl 是一个 **AI 驱动的 Kubernetes 运维助手**，用户可以通过自然语言与 Kubernetes 集群交互，无需记忆复杂的 kubectl 命令。
...
├── scripts/                  # 运维辅助脚本
├── package.json              # 项目配置文件
└── ~/.cubectl/.env           # 全局配置 (OPENAI_API_KEY 等)
```
