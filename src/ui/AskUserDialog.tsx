  ...
  return (
    <Box flexDirection="column">
      ...
      <Box marginTop={1} flexDirection="row">
        ...
        {value.length === 0 ? (
          <Text color="#888888">{placeholder || "Type your answer..."}</Text>
        ) : (
          <Text>{value}</Text>
        )}
      </Box>
...
const ChoiceQuestionView: React.FC<{
  ...
  const [selectedIndices, setSelectedIndices] = useState<Set<number>>(
    new Set(
      initialValues
        ? options
            .map((opt, i) => (initialValues.includes(opt.label) ? i : -1))
            .filter((i) => i !== -1)
        : [],
    ),
  );
  ...
  return (
    <Box flexDirection="column">
      <Text bold>
        {question}{" "}
        {multiple && (
          <Text color="gray">(Space to select multiple, Enter to confirm)</Text>
        )}
      </Text>
        ...
        {questions.map((q, i) => {
          ...
          return (
            <Box key={i} marginBottom={1}>
              ...
              <Text color={ans ? "white" : "red"}>
                {ans ? ans.join(", ") : "(unanswered)"}
              </Text>
            </Box>
          );
        })}
    ...
    if (key.escape) {
      ...
      if (
        ...
        (answeredIndices.has(currentIndex) ||
          currentIndex < request.questions.length - 1)
      ) {
        setCurrentIndex((prev) => prev + 1);
      }
      ...
      >
        ...
        <Box marginTop={1} flexDirection="column">
          ...
          <Text color="cyan" bold>
            Press Enter to submit everything
          </Text>
        </Box>
      </Box>
    );
  ...
  return (
    ...
    >
      ...
      <Box marginTop={1} flexDirection="column">
        ...
        <Text color="cyan" bold>
          Press Enter to{" "}
          {q.type === "text" ? "confirm answer" : "select option"}
        </Text>
      </Box>
    </Box>
  );
