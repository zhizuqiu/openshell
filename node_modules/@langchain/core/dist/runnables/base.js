...
var Runnable = class extends Serializable {
	...
	*/
	...
	async _callWithConfig(func, input, options) {
		...
		let output;
		try {
			output = await raceWithSignal(func.call(this, input, config, runManager), options?.signal);
		} catch (e) {
...
var RunnableSequence = class RunnableSequence extends Runnable {
	...
	async invoke(input, options) {
		...
		try {
			...
			for (let i = 0; i < initialSteps.length; i += 1) nextStepInput = await raceWithSignal(initialSteps[i].invoke(nextStepInput, patchConfig(config, { callbacks: runManager?.getChild(this.omitSequenceTags ? void 0 : `seq:step:${i + 1}`) })), options?.signal);
