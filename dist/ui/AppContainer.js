...
function Separator() {
    return (_jsx(Box, { borderStyle: "single", borderTop: true, borderBottom: false, borderLeft: false, borderRight: false, height: 1 }));
}
export function AppContainer({ config }) {
    ...
    const [messages, setMessages] = useState([]); // 聊天消息历史，包含用户输入、AI 响应和工具调用结果
    ...
    const [agent, setAgent] = useState(null); // LangChain Agent 实例
    ...
    const currentAiMsgIndexRef = useRef(-1); // 当前正在执行流式更新的 AI 消息在数组中的索引
    ...
    const seenMessageIdsRef = useRef(new Set()); // 已渲染的消息 ID 集合，用于多轮对话的消息去重
    ...
    const messageHasInterrupt = (m) => {
        return (m.role === Role.ASSISTANT &&
            Array.isArray(m.content) &&
            m.content.some((block) => block.type === MsgType.TOOL_CALL &&
    ...
    // --- 指令与流处理入口 ---
    // 处理用户提交的指令并启动 AI 流响应
    const handleCommand = async (command, isFromQuery = false) => {
...
${t('help.withAiAgent')}`,
    ...
    const handleAiStream = async (cmd) => {
    ...
    const processAiStream = async (stream) => {
        ...
        for await (const chunk of stream) {
            ...
            if (nodeData && Array.isArray(nodeData.messages)) {
                nodeData.messages = nodeData.messages.filter((msg) => {
            ...
            const interrupt = chunk['__interrupt__']?.[0] ||
                (firstMsg instanceof AIMessage
                    ? firstMsg.additional_kwargs['interrupts']?.[0]
                    : null) ||
                firstMsg?.interrupt;
                ...
                setMessages((prev) => {
                    ...
                    for (const block of assistantContent) {
                        if (block.type === MsgType.TOOL_CALL && block.tool_calls) {
                            const tc = block.tool_calls.find((t) => !t.result &&
            ...
            for (const msg of nodeData.messages) {
                ...
                if (!msgType && Array.isArray(msg.id)) {
                    ...
                    if (lcClass === 'HumanMessage' || lcClass === 'HumanMessageChunk')
                        ...
                        msgType = 'ai';
                    else if (lcClass === 'ToolMessage')
                        msgType = 'tool';
                    else if (lcClass === 'SystemMessage')
                        msgType = 'system';
                }
                // 统一角色判定
        ...
        try {
            if (agent) {
                ...
                if (history?.values?.messages) {
                    history.values.messages.forEach((msg) => {
                        const msgId = msg.id;
                        if (msgId)
                            seenMessageIdsRef.current.add(msgId);
                    });
    ...
    const handleDecision = async (decision, toolId, interrupt) => {
        if (!agent || !interrupt)
            return;
        activeStreamsRef.current++;
        setIsProcessing(true);
        ...
        try {
            ...
            const stream = await agent.stream(new Command({ resume: { [interrupt.id]: { decisions } } }), { streamMode: 'updates', configurable: { thread_id: 'main-session' } });
            await processAiStream(stream);
        }
        catch (error) {
            handleError(error);
    ...
    useEffect(() => {
        ...
        const handleKey = (key) => {
            ...
            if (key.name === 'right') {
                const nextPos = Math.min(currentInputValue.length, currentCursorPosition + 1);
                cursorRef.current = nextPos;
                setCursorPosition(nextPos);
                return;
            }
            ...
            if (key.name === 'backspace') {
                if (currentCursorPosition > 0) {
                    const newValue = currentInputValue.slice(0, currentCursorPosition - 1) +
            ...
            if (key.name === 'delete') {
                if (currentCursorPosition < currentInputValue.length) {
                    const newValue = currentInputValue.slice(0, currentCursorPosition) +
    ...
    if (pendingInterruptMessage &&
        ...
        const block = pendingInterruptMessage.content.find((b) => b.type === MsgType.TOOL_CALL &&
    ...
    return (_jsxs(Box, { flexDirection: "column", paddingX: 1, children: [_jsx(Static, { items: ['banner'], children: (item) => (_jsxs(Box, { marginBottom: 1, flexDirection: "column", alignItems: "center", width: "100%", children: [_jsx(Gradient, { name: "morning", children: _jsx(BigText, { text: "OPENSHELL", font: "block" }) }), _jsxs(Box, { marginTop: 1, flexDirection: "row", gap: 2, children: [_jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: "cyan", bold: true, children: "Enter" }), _jsx(Text, { dimColor: true, children: "\u53D1\u9001" })] }), _jsx(Text, { dimColor: true, children: "|" }), _jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: "cyan", bold: true, children: "Ctrl+A" }), _jsx(Text, { dimColor: true, children: "\u81EA\u52A8\u6A21\u5F0F" })] }), _jsx(Text, { dimColor: true, children: "|" }), _jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: "cyan", bold: true, children: "\u2191/\u2193" }), _jsx(Text, { dimColor: true, children: "\u5386\u53F2" })] }), _jsx(Text, { dimColor: true, children: "|" }), _jsxs(Box, { flexDirection: "row", gap: 1, children: [_jsx(Text, { color: "cyan", bold: true, children: "Ctrl+C" }), _jsx(Text, { dimColor: true, children: "\u9000\u51FA" })] })] })] }, item)) }, "brand-banner"), isLoading ? (_jsx(Box, { flexDirection: "column", marginY: 1, children: _jsxs(Box, { flexDirection: "row", alignItems: "center", gap: 1, children: [_jsx(Spinner, { type: "dots" }), _jsxs(Text, { children: [t('app.initializing'), "..."] })] }) })) : (_jsxs(_Fragment, { children: [_jsx(Static, { items: stableMessages, children: (msg) => (_jsx(MessageComponent, { message: msg }, `${msg.timestamp.getTime()}-${Math.random()}`)) }, "static-history"), activeMessages.map((msg) => (_jsx(MessageComponent, { message: msg }, `${msg.timestamp.getTime()}-${Math.random()}`))), _jsxs(Box, { flexDirection: "column", marginTop: activeMessages.length > 0 || stableMessages.length > 0 ? 1 : 0, children: [_jsxs(Box, { flexDirection: "row", justifyContent: "space-between", children: [_jsx(Box, { flexDirection: "row", alignItems: "center", gap: 1, children: _jsxs(Text, { bold: true, color: "cyan", children: ["\uD83D\uDE80 OpenShell ", config.version] }) }), _jsx(Box, { flexDirection: "row", gap: 2, children: _jsxs(Text, { color: "magenta", children: [t('status.autoExecuteLabel'), "(Ctrl+A):", ' ', autoExecute ? '✓' : '✗'] }) })] }), _jsx(Separator, {}), _jsxs(Box, { flexDirection: "column", marginTop: 1, marginBottom: 1, children: [pendingInterrupt ? (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Text, { color: "yellow", bold: true, children: ["Review Required:", ' ', pendingInterrupt.value?.action_requests?.[0]
                                                        ?.description || 'Action requires approval'] }), _jsx(Box, { marginTop: 1, children: _jsx(SelectInput, { items: [
                                                        { label: 'Approve', value: 'approve' },
                                                        { label: 'Reject', value: 'reject' },
                                                    ], onSelect: (item) => handleDecision(item.value, pendingToolId, pendingInterrupt) }) })] })) : (_jsxs(Box, { flexDirection: "row", alignItems: "center", children: [_jsx(Text, { color: isProcessing ? 'gray' : 'white', bold: true, children: '> ' }), _jsx(Box, { flexDirection: "row", children: isProcessing ? (inputValue ? (_jsx(Text, { dimColor: true, children: inputValue })) : (_jsx(Text, { color: "yellow", children: _jsx(Spinner, { type: "dots" }) }))) : (_jsxs(_Fragment, { children: [_jsx(Text, { children: inputValue.slice(0, cursorPosition) }), _jsx(Text, { inverse: true, children: inputValue[cursorPosition] || ' ' }), _jsx(Text, { children: inputValue.slice(cursorPosition + 1) })] })) })] })), suggestions.length > 0 && (_jsx(Box, { flexDirection: "column", marginTop: 1, paddingLeft: 2, borderStyle: "round", borderColor: "gray", children: suggestions.map((cmd, idx) => (_jsx(Box, { children: _jsxs(Text, { color: idx === selectedIndex ? 'cyan' : 'white', bold: idx === selectedIndex, children: [idx === selectedIndex ? '→ ' : '  ', "/", cmd] }) }, cmd))) }))] })] })] })), config.debug && (_jsx(Box, { marginBottom: 1, children: _jsxs(Text, { color: "yellow", children: ["DEBUG: ", t('app.debugMode')] }) }))] }));
}
